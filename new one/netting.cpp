...
//        game->ui->reproduce->hide();
//        game->ui->restart->hide();
//        game->ui->report->hide();
//game->lb->close();
//        game->ui->setbutton->hide();
        game->ui->report->close();
        net->havesend_end_op=false;
        net->received_give_up_op=false;
//        game->ui->lcdNumber->display(game->time_now);
         //memset(game->m_items,0,sizeof(game->m_items));
        memset(items,0,sizeof(items));
//        memset(items,0,sizeof(items));
        already_connected=true;
        //qDebug()<<already_connected;
        game->time_now=game->remaining_time=30;
        //qDebug()<<"ready";
        //qDebug()<<id<<sendid;
        if(id!=sendid) {
            if(data.data2=='w'){
...
//            Chat * chat=new Chat;
    ...
    case OPCODE::MOVE_OP:
        ...
        your_turn=true;
        game->lastMove=QPoint(x,y);
        emit game->turnNextPlayer(game->nextPlayer);
