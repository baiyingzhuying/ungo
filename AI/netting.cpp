...
//            }
        ...
        if(game->USE_AI==true){
            ...
            items[x][y]=real_color;
            QString move_x=QString::number(x);
            QString move_y=QChar(y+'A'-1);
            QString move_op=move_y+move_x;
//            QString times_op=QString::number(timestamp);
            this->server->send(lastOne,NetworkData(OPCODE::MOVE_OP,move_op,""));
        }
...
//            }
        ...
        if(game->USE_AI==true){
            ...
            QString move_op=move_y+move_x;
            //            QString times_op=QString::number(timestamp);
            this->socket->send(NetworkData(OPCODE::MOVE_OP,move_op,""));
        }
        info = time + " " + "Opponent" + " " +"MOVE_OP "+data.data1+ " "+data.data2+"\n";
        }
        game->ui->quitButton->show();
